...
export async function unlockVault(page: Page): Promise<void> {
  await login(page);
  // The vault picker may already be visible. Wait up to 20s for the
  ...
  // Cloud-vault unlock has its own testid — match either.
  const unlockBtn = page
    .locator('[data-testid="button-unlock-cloud-vault"], [data-testid="button-unlock-vault"], button:has-text("Unlock")')
    .first();
  ...
  await masterInput.waitFor({ state: 'detached', timeout: 30_000 }).catch(() => {});
}
